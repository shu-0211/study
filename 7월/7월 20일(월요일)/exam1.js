/*
    함수(Function) : 미리 정의된 수/코드  , 상자안에 들어오는 수/코드, 함(상자) 수(숫자/코드) 
        - 목적 : 1.재사용 2.매개변수에 따른 서로다른 결과물 3.지역변수
        * 매개변수(Parameter) : 함수호출하는 인자 와 함수실행하는 변수 연결 
        * 변수 : 하나의 값/자료 저장   , * 인자값 : 함수에 전달하는 값 
        - 종류 : 1. 미리만들어진함수(라이브러리) 2. 내가만든함수 
*/

// [1] 사용법 
// 1. 미리만들어진함수 : 라이브러리 , 각 프로그래밍 회사에서 제공 * 함수호출만
console.log( "함수1" )
// document.querySelector('body').innerHTML = '함수2'

// 2. 내가만드는함수 : 정의함수 , 개발자(내가) 함수 직접 정의 , * 함수도 자료의 타입 이다.
// * 함수정의 
...
// * 함수호출 
// 5) 호출할함수명   6) ( 인자값 )  ,   * 3 이라는 인자값이 매개변수1 에 저장된다.
func1( 3 ); // 
...
let 컵1 = 믹서기함수( '사과' ) // 믹서기함수에 '사과'를 인자값으로 전달 하여 '사과주스' 반환하여 컵1에 저장 
let 컵2 = 믹서기함수( '포도' ) // 믹서기함수에 '포도' 인자값으로 전달 하여 '포도주스' 반환하여 컵2에 저장

// [3] 예시2 : 수학공식함수는 x 와 y 매개변수로 받아서  
function 수학공식함수( x , y ){
    a = x + y   // 더하기  
    return a    // 반환 // 오직 하나만 나오는 규칙 *반환값 1번이면서 자료1개
} // f end 
let 결과1 = 수학공식함수( 3 , 5 )  // x=3 , y = 5 , 8
let 결과2 = 수학공식함수( 3 , -2 ) // x=3 , y = -2 , 1
// ==========================================================================================
// 1. 매개변수x , 반환값x 
function func2( ){ console.log( 'func2 exe' ); y = 3 + 2; }
func2( )
...
// 4. 매개변수x , 반환값o     예] 추후에 다양한 함수들이 사용되는데 내가만든 함수가 아니므로 무엇을 반환하는지 공문서/AI/구글 확인
...
// console.log( 지역변수1 ) // 오류O : 지역변수1 은 if{} 선언 했으므로 그 안에서만 사용가능
