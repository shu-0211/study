...
function productAdd(){
    ...
    let image = document.querySelector('.image').files[0]   // 첨부파일은 .files 속성에서 첨부파일 자료 가져오기
        // 유효성 판단 / 검사 
    ...
    // 2. 입력받은 값들을 객체화   : console.log() 를 이용하여 변수/자료 출력
        // pcode : 제품코드식별코드로 사용자 지정하지 않고 " 자동번호 부여 " , 마지막사용된 제품 코드에 + 1
        // pdate : 현재 시스템 날짜 / 시간 함수 , new Date().getFullYear()
    ...
    let object = {ccode : category , pname : name , pprice : price , 
                    // 만약에 첨부파일선택이 없으면(undefined) 기본이미지 (https://placehold.co/100) 있으면 선택한 첨부파일 가상 URL 생성
                    pimg : image == undefined ? 'https://placehold.co/100' : URL.createObjectURL(image) ,  // 입력받은 이미지정보를 가상URL 생성 
                    pcode : finalpcode+1 , pdate : pdate}
    console.log(object)

    // 3. 배열 저장 
    productList.push(object); finalpcode++;
